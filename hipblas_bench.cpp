...
int main(int argc, char **argv) {
    ...
    hipMalloc((void **)&A_dev, mat_sz);
    hipMalloc((void **)&B_dev, mat_sz);
    hipMalloc((void **)&C1_dev, mat_sz);
    hipMalloc((void **)&C2_dev, mat_sz);

    stat = hipblasCreate(&handle);

    stat = hipblasSetMatrix(N, N, sizeof(realtype), A, N, A_dev, N);
    stat = hipblasSetMatrix(N, N, sizeof(realtype), B, N, B_dev, N);

    // hipMemcpy(A_dev, A, mat_sz, hipMemcpyHostToDevice);
    // hipMemcpy(B_dev, B, mat_sz, hipMemcpyHostToDevice);
    ...
    for (int i = 0; i < rep_num; i++) {
        ...
        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, N, N, N, 1.0, A,
                    N, B, N, 0.0, C1, N);
        */
        stat = hipblasSgemm(handle, HIPBLAS_OP_N, HIPBLAS_OP_N, N, N, N, &alpha,
        ...
        cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, N, N, N, 1.0, A,
                    N, B, N, 0.0, C1, N);
        */
        stat = hipblasDgemm(handle, HIPBLAS_OP_N, HIPBLAS_OP_N, N, N, N, &alpha,
                            A_dev, N, B_dev, N, &beta, C1_dev, N);
#endif
    }
    ...
    for (int i = 0; i < rep_num; i++) {
        custom_kernel_10_call(A_dev, B_dev, C2_dev, N, N, N, alpha, beta);
    }

    hipDeviceSynchronize();
    gettimeofday(&stop2, NULL);

    //hipMemcpy(C1, C1_dev, mat_sz, hipMemcpyDeviceToHost);
    stat = hipblasGetMatrix(N, N, sizeof(realtype), C1_dev, N, C1, N);
    hipMemcpy(C2, C2_dev, mat_sz, hipMemcpyDeviceToHost);
    ...
    */
    ...
    printf("Total time elapsed for hipblas is %f\n", sec_elapsed_1);
    ...
    printf("Resulting hipblas GFLOPS = %f\n", gflops_1);
